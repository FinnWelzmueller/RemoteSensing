...
df = pd.read_csv("./correlation-data.csv", delim_whitespace=True)
...
ax.set_xlim(0,1.3)
ax.set_ylim(0,12.5)
ax.annotate(fr"$r = {r}$", xy=(0.1,0.9),xycoords='axes fraction', fontsize=14)
fig.savefig("correlation-plot.png")

#plot without outlier (station 12)
df = pd.read_csv("./correlation-data-no-outlier.csv", delim_whitespace=True)
r = np.around(pearsonr(df["Chla_[mg/m3]_ref"], df["CHL1_mean_mean"])[0],3)
print(r)
fig, ax = plt.subplots()
#ax.grid()
...
ax.annotate(fr"$r = {r}$", xy=(0.1,0.9),xycoords='axes fraction', fontsize=14)
fig.savefig("correlation-plot-no-outlier.png")